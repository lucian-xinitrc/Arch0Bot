		...
		try:
			if ctx.author.id == 1135659932000202942 or ctx.author.id == 1027255470429319228:
				...
				if result['status'] == "started":
					...
					await ctx.response.send_message(content = "@everyone, The server just started!", allowed_mentions = allowed_mentions)
				else:
	...
	async def command_mc_server(inter, ctx, type: types, command):
		...
		try:
			...
			if ctx.author.id == 1135659932000202942 or ctx.author.id == 1027255470429319228:
				...
				response = requests.post('http://91.99.202.74:8888/api/insertCustomCommand', json=data)
				result = response.json()
		...
		try:
			if ctx.author.id == 1135659932000202942 or ctx.author.id == 1027255470429319228:
				...
				if result['status'] == "closed":
					allowed_mentions = discord.AllowedMentions(everyone = True)
					await ctx.response.send_message(content = "@everyone, The server is closed now!", allowed_mentions = allowed_mentions)
			else:
				await ctx.response.send_message("You have no permission!")
		except:
			await ctx.response.send_message("There was an error!")
